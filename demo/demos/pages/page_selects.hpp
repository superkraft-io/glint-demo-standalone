...
inline void glint_demos_window::buildSelects()
{
  ...
  auto* sel1 = mContent->add.fromClass<glint_select>([](glint_select& sel1) {
    sel1.options = { "Option A", "Option B", "Option C", "Option D" };
    sel1.selectedIndex = 0;
    sel1.style.width = 220.f;
  ...
  mContent->add.fromClass<glint_select>([](glint_select& sel3) {
    sel3.options = { "Attack", "Decay", "Sustain", "Release" };
    sel3.selectedIndex = 0;
    sel3.style.width = 220.f;
  ...
  auto makeSmall = [&](glint_element* parent, std::vector<std::string> opts, int sel) {
    return parent->add.fromClass<glint_select>([opts = std::move(opts), sel](glint_select& s) mutable {
  ...
  auto* row = mContent->add.div([](glint_component_style& row) {
